...
public interface AdminDashboardStatsMapper {
    ...
    @Select("SELECT COUNT(*) FROM fc_alert_record WHERE created_at >= DATE_SUB(NOW(), INTERVAL #{days} DAY)")
    long countAlertsInDays(@Param("days") int days);

    @Select("SELECT health_score FROM fc_health_report WHERE report_date >= DATE_SUB(NOW(), INTERVAL #{days} DAY)")
    List<Integer> listHealthScoresInDays(@Param("days") int days);

    @Select("SELECT risk_score FROM fc_health_report WHERE report_date >= DATE_SUB(NOW(), INTERVAL #{days} DAY)")
